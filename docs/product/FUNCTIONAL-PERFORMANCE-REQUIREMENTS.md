...
| 关键词发现 | 通过 Brave Search 或 SearXNG 搜索新闻和公开网页；按专题词初筛相关性；保存全部查询历史和候选结果 |
| 选择性抓取 | 只处理用户选择或专题启动时初筛为明确相关的结果；逐条显示处理状态；安全直抓质量不足时可使用明确配置的 Reader |
...
| 编号 | 指标 | 当前状态 | P1 要求 |
| --- | --- | --- | --- |
| F-01 | 专题隔离 | 已实现 | 材料、搜索、任务、事件、来源和报告默认归属当前专题；首页汇总与专题队列明确区分 |
| F-02 | 关键词搜索 | 已实现 | 支持新闻和一般网页搜索；按专题词与事件时间范围解释相关性初筛；范围待核对项不自动进入处理；全部候选和查询历史可重开；跨页去重 |
| F-03 | 选择性抓取 | 已实现 | 用户选择或专题启动时初筛为明确相关后逐条抓取原文；存疑和可能无关结果默认不进入待处理；单条失败不影响其他条目 |
...
| F-08 | AI 草稿 | 已实现 | 结合专题问题生成有限数量的相关性、事件、实体、关键信息和原文依据候选；临时失败时保留原文并自动退避重试，多次失败后允许手动重新分析，不伪造成功草稿 |
...
| F-21 | 综合报告 | 已实现 | 正文按结论与真实事件组织，关键发现直接展示原文片段，网页集中作为证据引用和来源附录，不逐篇复述 |
...
| 单次抓取正文上限 | 5 MB |
| 抓取总超时 | 30 秒；直抓默认最多占用 12 秒，为 Reader 兜底保留时间 |
| 正文提取 | Trafilatura 主提取并保留段落定位；直抓固定到已校验公网 IP；质量不足时可启用 Jina Reader 浏览器渲染；代理网络可用可信 DoH 独立验证目标，任一非公网结果或 Reader 失败均失败关闭；启用者须信任第三方 Reader 自身的 SSRF 防护 |
| 单个 RSS 每次处理条目 | 50 条 |
| P1 真实来源规模 | 30–50 个 |
| P1 连续运行 | 72 小时，并输出运行报告 |
| 任务恢复 | 进程重启后队列保持；过期任务可恢复 |
| 后台并发 | 默认 4 个受控工作槽，可在 1–32 范围配置；单条失败不阻塞其他任务 |
| 模型并发与输出 | 默认最多 1 个模型请求并发；候选输出默认限制 1400 tokens、最多 5 个实体、3 条关键信息、每条 2 个原文依据，优先保证完整返回和用户可读性 |
...
2026-09-03 使用 8 组中英文关键词取得 120 个公开 URL，覆盖 84 个域名；以 12 路并发执行抓取与正文质量检查，不写入专题、不调用模型，单条总上限 30 秒。同一 URL 池结果如下：

| 采集版本 | 可用正文 | 失败或质量不足 | 说明 |
| --- | ---: | ---: | --- |
| 修复前线上版本 | 64 / 120 | 56 / 120 | 本机代理 DNS 导致 9 条安全拦截，另有 26 条超时 |
| 可信 DoH、Reader 代理和跳转兜底候选 | 86 / 120 | 34 / 120 | 54 条直抓、32 条 Reader；剩余包括 19 条超时、9 条来源/Reader 拒绝、5 条正文不足、1 条格式不支持 |

该抽样证明修复没有靠放宽私网规则换取成功率，并将可用率从 53.3% 提高到 71.7%。它是高并发异构网页抽样，不是“任意网站必须抓取成功”的承诺；重复测试可能命中 Reader 短时缓存。P1 的正式退出仍须冻结 30–50 个真实来源并完成 72 小时运行。
