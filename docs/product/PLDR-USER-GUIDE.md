...
首页右侧是 **全部专题待处理**，用于汇总所有专题并明确标出归属；进入某个专题后，“待处理”只读取该专题的任务。未归类材料和其他专题的任务不得混入专题队列。
...
如果 AI 超时或返回无效结果，页面显示 **AI 分析失败/超时** 和 **重新分析**。原文仍在，重新分析不会再次抓取网页；系统也不会用规则摘录冒充成功的 AI 结果。没有配置模型时才会明确显示规则模式。
...
报告只读取生成时已经确认的正式对象，依次展示当前结论、关键发现、按真实事件归并的脉络、信息缺口和来源附录；网页只作为引用依据，不再逐篇重复成报告正文。
