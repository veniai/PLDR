...
7. 每条结果带有“与专题相关 / 相关性存疑 / 可能无关”的初筛说明。初筛只读取专题词、标题、搜索摘要，以及搜索服务提供或摘要中明确标注的发布日期；不会把正文中没有发布标记的事件日期冒充发布时间，也不是事实核验或来源可靠性评分。早于专题事件范围的材料标为“时间范围待核对”；存疑、可能无关和时间待核对结果仍可查看、筛选和手动提交。
