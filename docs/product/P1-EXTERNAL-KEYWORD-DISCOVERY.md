...
7. 每条结果带有“与专题相关 / 相关性存疑 / 可能无关”的初筛说明。初筛只读取专题词、标题、搜索摘要，以及搜索服务提供、摘要明确标注或新闻地址完整编码的年月日；不会把正文中没有发布标记的事件日期或链接查询参数冒充发布时间，也不是事实核验或来源可靠性评分。早于专题事件范围的材料标为“时间范围待核对”；存疑、可能无关和时间待核对结果仍可查看、筛选和手动提交。
...
4. 模型超时或临时返回非法输出时保留原文并自动重新排队，默认退避重试 3 次；期间不要求用户操作，也不用规则摘录冒充成功的 AI 候选。多次重试仍失败后才显示可手动重新分析，且不重复抓取。仅在没有配置模型时才明确使用确定性规则模式。无法形成可定位证据的条目不得确认。
