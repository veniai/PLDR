...
工作台的“今天”页按待审核、处理中、失败和已确认事件组织注意力；“发现资料”保留搜索产生的任务；“待我审核”是正式入档前的操作入口。它们读取同一专题的持久状态，不用前端临时成功提示替代服务端结果。
...
4. 模型超时或非法输出时可以保留确定性降级候选，并明确标记为降级；降级不是正式结论，仍须人工审核。无法形成可定位证据的条目不得确认。
