...
它复用 P0.3 的候选与人工确认闭环，不再另做一套审核系统。**完成本切片不代表完整 P1 已退出**：30–50 个来源、连续 72 小时运行、PostgreSQL/PostGIS、模型调用成本和验收报告仍是后续 P1 工作。

## 对象和边界

- `CollectionTarget` 是“要定期检查哪个公共网页”的运行配置，不是正式 `Source`。
...
- RSS/API/需要登录的来源、浏览器渲染和 Browser Steps。
